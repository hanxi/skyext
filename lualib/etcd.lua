...
local function _request_uri(self, host, method, uri, opts, timeout, ignore_auth)
    ...
    log.debug("request_uri end", "uri", uri, "status", status, "body", body, "host", host)
    if status >= 500 then
        log.warn("request_uri failed", "uri", uri, "status", status, "body", body, "host", host)
...
local function report_failure(self, http_host)
    fail_expired_time[http_host] = now() + fail_timeout
    self.working_endpoint = nil
end
...
local function get_working_endpoint(self)
    local endpoint = self.working_endpoint
    if not endpoint then
        endpoint = choose_endpoint(self)
...
function refresh_jwt_token(self, timeout)
    ...
    if self.jwt_token and now() - self.last_auth_time < 60 * 3 + random(0, 60) then
        log.debug("refresh_jwt_token no need refresh")
        return true, nil
    end
    ...
    if self.requesting_token then
        ...
        if self.jwt_token and now() - self.last_auth_time < 60 * 3 + random(0, 60) then
            log.debug("refresh_jwt_token no need refresh after wait")
            return true, nil
        end

        if self.last_refresh_jwt_err then
            log.warn("refresh jwt last", "err", self.last_refresh_jwt_err)
            return nil, self.last_refresh_jwt_err
        end

        -- something unexpected happened, try again
        log.info("try auth after waiting", "timeout", timeout)
    end
    ...
    if err then
        ...
        log.warn("refresh_jwt_token failed", "err", err)
        return nil, err
    end
    ...
    log.info("refresh_jwt_token succ", "token", self.jwt_token)
    return true, nil
end
...
function _M.txn(self, compare, success, failure, opts)
    ...
    if success then
        ...
        for i, rule in ipairs(success) do
            ...
            if rule.requestRange then
                local requestRange = tab_clone(rule.requestRange)
                requestRange.key = encode_base64(get_real_key(self.key_prefix, requestRange.key))
                rule.requestRange = requestRange
            end
            new_rules[i] = rule
        end
        success = new_rules
    ...
    if failure then
        local new_rules = tab_clone(failure)
        for i, rule in ipairs(failure) do
            rule = tab_clone(rule)
            if rule.requestPut then
                local requestPut = tab_clone(rule.requestPut)
                requestPut.key = encode_base64(get_real_key(self.key_prefix, requestPut.key))
...
local watch_mt = {
    __call = function(self)
        ...
        if stream.status ~= 200 then
            log.warn("watch response faile", "resp", resp)
            return nil, stream.status
        end

        local body = decode_json(resp)
        if not body then
            log.error("decode json failed", "resp", resp)
            report_failure(self, self.endpoint.http_host)
            return nil, "decode json failed"
        end

        if body.error then
            log.warn("watch return error", "err", body.error)
...
function _M.refresh_jwt_token(self, timeout)
    timeout = timeout or 5
    return refresh_jwt_token(self, timeout)
end
